...
const UI = (() => {
  ...
  function screenFrame(title, w, h) {
    ...
    Art.textCentred(En, x + w / 2, y + 14, title, Core.idx(13, 14), 2);
  ...
  function defeat(g) {
    ...
    Art.textCentred(En, En.W / 2, 158, 'YOUR PARTY HAS FALLEN', Core.idx(11, 13), 3);
    Art.textCentred(En, En.W / 2, 210, 'The priests of Harrowgate will take you in,', Core.idx(0, 13), 2);
    Art.textCentred(En, En.W / 2, 232, 'for a share of what you carry.', Core.idx(0, 13), 2);
  ...
  function title(g) {
    ...
    // DITHERED, like the in-game sky. Rounding a nine-step gradient to an integer shade gives seven
    // flat 43-pixel bands with one colour per row across the whole width — the exact defect that was
    // fixed in every 3D view, still alive on the first screen anybody sees.
    const TH = [0.0625, 0.5625, 0.1875, 0.6875, 0.4375, 0.9375, 0.3125, 0.8125];
    for (let y = 0; y < En.H; y++) {
      const exact = clamp(3 + y / En.H * 9, 0, 15);
      const lo = Math.floor(exact), frac = exact - lo;
      for (let x = 0; x < En.W; x++) {
        En.px(x, y, Core.shade(9 << 4, frac > TH[(x + (y & 1) * 4) & 7] ? lo + 1 : lo));
      }
    }
    for (let x = 0; x < En.W; x++) {
    ...
    const midX = En.W / 2;
    Art.textCentred(En, midX + 2, 66, 'THORNMARCH', Core.idx(0, 1), 5);
    Art.textCentred(En, midX, 64, 'THORNMARCH', Core.idx(13, 6), 5);
    Art.textCentred(En, midX - 1, 63, 'THORNMARCH', Core.idx(13, 15), 5);
    const tw = Art.textWidth('THORNMARCH', 5);
    En.hline(midX - tw / 2, 112, tw, Core.idx(13, 11));
    En.hline(midX - tw / 2, 114, tw, Core.idx(13, 5));
    Art.textCentred(En, midX + 1, 127, 'THE ASHEN CROWN', Core.idx(0, 1), 2);
    Art.textCentred(En, midX, 126, 'THE ASHEN CROWN', Core.idx(13, 14), 2);

    // Centred on the frame, not on 320 — the title screen's own buttons were sitting a hundred
    // pixels left of centre after the framebuffer went to 800 wide.
    const bx0 = Math.round(midX - 120);
    Art.button(En, bx0, 250, 240, 56, 'NEW GAME', false, 3);
    reg('newgame', bx0, 250, 240, 56, 1);
    Art.button(En, bx0, 316, 240, 56, 'CONTINUE', false, 3);
    reg('continue', bx0, 316, 240, 56, 1);
    Art.textCentred(En, midX, 444, 'THE ROAD ENDS AT THE KEEP', Core.idx(13, 11), 2);
  }
