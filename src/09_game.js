...
const Game = (() => {
  ...
  function castSpell(id) {
    ...
    // WORLD- AND ITEM-SCOPE SPELLS HAVE NO TARGET LIST, AND THAT IS NOT AN ERROR. Spellcraft
    // resolves them from the caster alone and every one of their effect handlers already exists;
    // this guard ran first and turned all seven into "No target." The guild sold five of them
    // anyway, for 29,020 gold that could never do anything — Town Portal and Lloyd's Beacon among
    // them, which are exactly the spells a player saves up for.
    const scoped = sp.target === 'world' || sp.target === 'item';
    if (!scoped && !targets.length) { Log.push('No target.', 'info'); return false; }
  ...
  const SPECIALS = {
    ...
    enchant_item(eff) {
      ...
      // A non-finite power must not index the table. clamp(NaN) is NaN, ENCHANTS[NaN] is
      // undefined, and reading .bonus off it takes the whole page down mid-spell.
      const pw = Number.isFinite(eff.power) ? eff.power : 0;
      const tier = clamp(Math.floor(pw / 12) | 0, 0, Items.ENCHANTS.length - 1);
      const ench = Items.ENCHANTS[tier];
      if (!ench) { Log.push('The enchantment will not take.', 'info'); return false; }
      st.ench = tier;
      st.bonus = (st.bonus || 0) + ench.bonus;
      st.ident = true;
      recompute(ch);
  ...
  function load(slot) {
    ...
    try {
      ...
      for (const c of state.party.members) {
        ...
        // EVERY per-member object a caller dereferences without checking must exist. `cond` is the
        // one the validator missed: a save with a member missing it LOADED SUCCESSFULLY and then
        // invariants() and brief() both threw on `ch.cond.dead` — the game kept running while its
        // own introspection was dead, which is the worst of both outcomes. Rebuild rather than
        // reject: the rest of the character is fine and a missing status block means "healthy".
        // Rules.CONDITIONS is an ARRAY of names. Iterating its keys wrote cond[0..7] onto every
        // character and broke save/load byte-identity, which the campaign suite caught immediately.
        // Only fill in a condition that is genuinely absent, so a healthy save round-trips unchanged.
        if (!c.cond || typeof c.cond !== 'object') c.cond = {};
        for (const k of Rules.CONDITIONS) if (c.cond[k] === undefined) c.cond[k] = false;
        ...
        if (!Array.isArray(c.pack)) c.pack = [];
  ...
  function onTap(x, y, down) {
    ...
    switch (r.id) {
      ...
      // five cells away, ten gold bought 31/18/12/21 HP back and the wolf never moved. The door
      // refused at 3.2 cells while camping refused at 14, so there was an eleven-cell band where
      // the exploit lived. One rule now answers both.
      case 'tavernrest': {
        const blocker = safeToRest();
        if (blocker !== true) {
          Log.push(Rules.canRest(state.party, blocker).why, 'info');
        } else if (state.party.gold >= r.data) {
          state.party.gold -= r.data; Clock.skip(480); healParty();
          Log.push('You sleep at the inn.', 'good');
        } else Log.push('Not enough gold for a bed (' + r.data + ').', 'info');
        break;
      }
